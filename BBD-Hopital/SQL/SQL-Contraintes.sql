...
if(p>:new.DATEPRESCRIPTION)then
    raise_application_error(-20666, 'On ne peut pas prescrire un traitement avant l''entrée du patient');
end if;
end;/
...
begin
   Select Sum(QUANTITEMEDOC) into quantite FROM COMPOSER Where IDTRAITEMENT = (SELECT ID From TRAITEMENT WHERE DATETRAITEMENT > SYSDATE-120) AND IDMEDICAMENT = :new.ID;
